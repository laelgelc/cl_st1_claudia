...
continuous qualitative description of the music recording using the Gemini API.

Source audio files are resolved from the input record's "audio_file" field when
available, or from the input directory as "<corpus_id>.wav". Description outputs
are written to the output directory as "<corpus_id>.txt" and "<corpus_id>.json".

The plain-text description is intended for corpus linguistic analysis and human
inspection. The JSON description preserves model configuration, source metadata,
and run metadata for reproducibility.

By default, the script runs in test mode and attempts only the first planned
music video. Existing description files are skipped unless --reprocess is provided,
...
Full run from a specific music video:
    python describe_music_videos_music.py --no-test-mode --start-corpus-id 4xqo7D2k8HM

The script writes an append-only log file, a JSON manifest, and a curated NDJSON
description index for downstream analysis.
"""
...
def parse_args() -> argparse.Namespace:
    """Parse command-line arguments for the music videos description programme."""
    parser = argparse.ArgumentParser(
        description="Describe music video audio with Gemini API."
    )
    ...
    parser.add_argument("--description-index-file", type=Path, default=Path(DEFAULT_DESCRIPTION_INDEX_FILE))
    ...
    args.description_index_file = resolve_script_relative_path(args.description_index_file)
...
def describe_one_item(
    ...
    """Uploads audio, generates description using Gemini, and cleans up."""
    ...
    for attempt in range(1, total_attempts + 1):
        try:
            logger.info("Description attempt %s/%s for %s", attempt, total_attempts, corpus_id)
            ...
            result_json = {
                ...
                "run": {
                    "generation_run_id": model_config["run_id"],
                    "generated_at_utc": end_time,
                },
                "status": "success",
                "error": None,
            }

            text_out.write_text(description_text + "\n", encoding="utf-8")
    ...
    return make_item_base(record) | {
        ...
        "json_output_path": str(json_out),
        "status": "failed",
        "error": final_error or "Description generation failed.",
        "retries": retries_used,
        "duration_seconds": duration,
        "start_time": start_time,
        "end_time": end_time,
...
def make_skipped_existing_result(item: dict[str, Any], model_config: dict[str, Any], logger: logging.Logger) -> dict[str, Any]:
    """Create a manifest item for a skipped complete description pair."""
...
def make_description_index_record(item: dict[str, Any], model_config: dict[str, Any]) -> dict[str, Any]:
    """Build one curated description index record."""
    ...
    record.update({
        ...
        "description_text_file": item.get("text_output_path"),
        "description_json_file": item.get("json_output_path"),
        "description_status": item.get("status"),
        "generation_run_id": model_config["run_id"],
        "generated_at_utc": item.get("end_time"),
        "model_name": model_config["model_name"],
...
def write_description_index(index_records: list[dict[str, Any]], index_file: Path) -> None:
    """Write curated NDJSON description index."""
...
def main() -> int:
    """Run the batch music video description workflow."""
    args = None
    logger = None
    run_id = make_run_id()
    start_time = utc_timestamp()
    ...
    try:
        ...
        planned, skipped_existing, missing_input = plan_descriptions(
        ...
        index_records = [
            make_description_index_record(item, model_config)
        ...
        write_description_index(index_records, args.description_index_file)
